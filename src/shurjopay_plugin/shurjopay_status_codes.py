...
SHURJOPAY_STATUS_CODES = {
    'AUTH_SUCCESS': (200, "Shurjopay authentication successful."),
...
# get the status code and message from the dictionary
def get_status_by_code(code):
    for key, value in SHURJOPAY_STATUS_CODES.items():
        if value[0] == code:
            return value[1]
